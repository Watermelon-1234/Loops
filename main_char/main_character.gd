...
# 狀態控制
enum State { MENU, READY, WALK, IDLE, JUMP, DAMAGED, DIE }
...
func _physics_process(delta: float) -> void:
	...
	if current_state == State.IDLE || current_state == State.READY || current_state == State.MENU:
		velocity.x = 0
		move_and_slide()
		_update_animation()
		return
...
func _check_special_tiles() -> void:
	if current_state == State.DAMAGED or current_state == State.DIE or current_state == State.READY or current_state == State.MENU:
		return
...
func _special_effect(feature: String) -> void:
	match feature:
		"super_jump":
			velocity.y = -250
		"respawn_point":
			GameManager.update_respawn_position(global_position)
...
func _animation_finished() -> void:
	debug(["_animation_finished"])
	if sprite.animation == "main_char_die":
...
func _update_animation() -> void:
	...
	if current_state == State.READY or current_state == State.MENU:
